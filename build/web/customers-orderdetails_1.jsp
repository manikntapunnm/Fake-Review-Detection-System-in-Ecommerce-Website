...
try {
   ...
   PreparedStatement statement = connection.prepareStatement(sql);
    statement.setString(1, cartid); // Assuming userEmail is defined somewhere
...
%>
									...
									<tr>
										...
										<td>
											<img src="img2/paypal.png" alt="" width="70px" height="40px">
											<span>**** **** 9099</span>
										</td>
...
try {
    ...
    while (resultSet.next()) {
        int orderid = resultSet.getInt("order_id"); // Corrected column name
...
try {
    Connection connection = new Dbconn.dbconn().getConnection(); // Remove "dbconn." before "getConnection()"
     ...
     String sql = "SELECT cart.*, products.*, customers.*, orders.* " +
                 ...
                 "JOIN customers ON cart.email = customers.email " +
                 "JOIN orders ON cart.email = orders.email " +
                 "WHERE cart.email=? AND cart.checkstatus='Checked'";
   PreparedStatement statement = connection.prepareStatement(sql);
    statement.setString(1, userEmail); // Assuming userEmail is defined somewhere
