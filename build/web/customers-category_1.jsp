...
<body>
	...
	<section class="category-section ">
		<div class="container">
			<!-- <div class="back-link">
				<a href="customers-checkout.jsp"> >> Next to Checkout Page</a>
			</div> -->
			<div class="row">
				<div class="col-lg-3 order-2 order-lg-1">
					
<%@ page import="java.sql.*, java.util.*, javax.servlet.*, javax.servlet.http.*, org.apache.commons.codec.binary.Base64" %>
<%
try {
    Connection connection = new Dbconn.dbconn().getConnection();

    // Retrieve distinct brand values from the products table
    String brandQuery = "SELECT DISTINCT brand FROM products";
    PreparedStatement brandStatement = connection.prepareStatement(brandQuery);
    ResultSet brandResultSet = brandStatement.executeQuery();

    // Use a Set to store unique brand values
    Set<String> uniqueBrands = new HashSet<>();
    while (brandResultSet.next()) {
        uniqueBrands.add(brandResultSet.getString("brand"));
    }

    // Close resources related to brand query
    brandResultSet.close();
    brandStatement.close();

    // Now you can use the unique brand values to dynamically populate your list
%>
<div class="col-lg-3 order-2 order-lg-1">
    <div class="filter-widget">
        ...
        <ul class="category-menu">
            <% for (String brand : uniqueBrands) { %>
                <li><a href="customers-category_1.jsp?brand=<%= brand %>"><%= brand %></a></li>
            <% } %>
        </ul>
    </div>
</div>
				...
				<div class="col-lg-4 col-sm-6">
					<div class="product-item">
						<div class="pi-pic">
							...
							<div class="pi-links">
									<a href="add-to-mycartcategory_1.jsp?productid=<%= resultSet.getInt("product_id")%>&email=<%=userEmail%>&price=<%=price%>&discount=<%=discount%>&brand=<%=brand%>" class="add-card"><i class="flaticon-bag"></i><span>ADD
										TO
										CART</span></a>
								<a href="add-to-mywishlistcategory_1.jsp?productid=<%= resultSet.getInt("product_id")%>&email=<%=userEmail%>&brand=<%=brand%>" class="wishlist-btn"><i
										class="flaticon-heart"></i></a>

<!--								<a href="customers-productdetailscategory_1.jsp?productid=<%= resultSet.getString("product_id")%>&brand=<%=brand%>" class="wishlist-btn"><i
										class="fa fa-eye"></i></a>-->
...
<!--						<div class="col-lg-4 col-sm-6">
							<div class="product-item">
								<div class="pi-pic">
									<a href="customers-productdetails.jsp"><img src="img2/cover3.jpg" width="250px"
											height="350px" alt=""></a>
									<div class="pi-links">
										<a href="customers-mycart.jsp" class="add-card"><i
												class="flaticon-bag"></i><span>ADD TO
												CART</span></a>
										<a href="customers-mywishlist.jsp" class="wishlist-btn"><i
												class="flaticon-heart"></i></a>

										<a href="customers-productdetails.jsp" class="wishlist-btn"><i
												class="fa fa-eye"></i></a>
									</div>
								</div>
								<div class="pi-text">
